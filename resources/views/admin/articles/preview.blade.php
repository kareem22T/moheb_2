...
<div class="card w-100" id="word_prev">
    <div class="card-header d-flex justify-content-between gap-3">
        ...
        <a href="/Moheb/admin/articles/add" class="btn btn-primary w-fit d-flex gap-2 align-items-center">
    ...
    <div class="table-responsive" v-if="articles_data && articles_data.length > 0">
        ...
        <tbody>
            <tr v-for="(article, index) in articles_data" :key="index">
                ...
                <td class="border-bottom-0"><h6 class="fw-semibold mb-0">@{{article.name}}</h6></td>
                <td class="border-bottom-0">
                    <p class="mb-0 fw-normal">@{{article.category.main_name}}</p>
                </td>
                <td class="border-bottom-0">
                    <div class="d-flex gap-2">
                        <a :href="`/Moheb/admin/articles/edit/${article.id}`" class="btn btn-secondary p-2"><h4 class="ti ti-edit text-light m-0 fw-semibold"></h4></a>
    ...
    <div class="pop-up delete_pop_up card w-50" style="margin: auto; display: none;"  :class="{ 'show': delete_pop_up }" v-if="delete_pop_up">
        <div class="card-body">
            <form @submit.prevent>
                <h5 class="mb-3 text-center">Are you sure you want to delete @{{ article_name }} article?</h5>
...
createApp({
    ...
    methods: {
        async update(lang_id, lang_symbol, lang_name) {
            ...
            try {
                const response = await axios.post(`/Moheb/admin/languages/edit`, {
                    lang_id: lang_id,
        ...
        async deletearticle(article_id) {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/Moheb/admin/articles/delete`, {
        ...
        async getarticles() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/Moheb/admin/articles?page=${this.page}`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
        ...
        async getSearch(search_words) {
            try {
                const response = await axios.post(`/Moheb/admin/articles/search?page=${this.page}`, {
