...
createApp({
    ...
    methods: {
        async add(symbol, lang_name) {
            ...
            try {
                const response = await axios.post(`/Moheb/admin/languages/add`, {
                    symbol: symbol,
                    name: lang_name,
                },
                );
                if (response.data.status === true) {
                    ...
                    setTimeout(() => {
                        $('.loader').fadeOut()
                        $('#errors').fadeOut('slow')
                        window.location.href = '/Moheb/admin/languages'
                    }, 2000);
                } else {
                    $('.loader').fadeOut()
                    document.getElementById('errors').innerHTML = ''
        ...
        async update(file, file_name) {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/Moheb/admin/languages/content/update`, {
                    file: file,
                    file_name: file_name,
                },
                {
