...
<div class="card w-100" id="lang_prev">
    <div class="card-header d-flex justify-content-between gap-3">
        ...
        <a href="/Moheb/admin/categories/add" class="btn btn-primary w-fit d-flex gap-2 align-items-center">
            ...
            <tr v-for="(category, index) in categories_data" :key="index">
                ...
                <td class="border-bottom-0">
                    <div class="d-flex gap-2">
                        <a :href="`/Moheb/admin/category/${category.id}`" class="btn btn-success p-2 edit_lang_btn"><h4 class="ti ti-eye text-light m-0 fw-semibold"></h4></a>
                        <a :href="`/Moheb/admin/categories/edit/${category.id}`" class="btn btn-secondary p-2"><h4 class="ti ti-edit text-light m-0 fw-semibold"></h4></a>
...
createApp({
    ...
    methods: {
        ...
        async deletCat(cat_id) {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/Moheb/admin/categories/delete`, {
                    cat_id: cat_id,
                },
                );
                if (response.data.status === true) {
        ...
        async getCategories() {
            $('.loader').fadeIn().css('display', 'flex')
            try {
                const response = await axios.post(`/Moheb/admin/categories?page=${this.page}`, {
                },
                );
                if (response.data.status === true) {
                    $('.loader').fadeOut()
        ...
        async getSearch(search_words) {
            try {
                const response = await axios.post(`/Moheb/admin/categories/search?page=${this.page}`, {
